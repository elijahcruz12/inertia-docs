...
<Steps>
    ...
    <Step title="Setup root template">
        ...
        The `<x-inertia::head>` component also accepts fallback content via its slot. This content only renders when [SSR](/v3/advanced/server-side-rendering) is not active, preventing duplicate tags.
        ...
        <Accordion title="Using Blade directives">
            Inertia also provides `@inertiaHead` and `@inertia` Blade directives as an alternative to the Blade components. These directives do not support SSR head fallback content.
