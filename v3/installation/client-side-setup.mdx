...
By default, Inertia passes the initial page data via a `<script type="application/json">` element. You may revert to the legacy behavior of storing page data in a `data-page` attribute on the root element by setting the `legacy.useDataAttributeForInitialPage` option to `true`.
...
Laravel users should also set the `inertia.pages.use_data_attribute_for_initial_page` config value to `true` so the `@inertia` Blade directive outputs a data attribute instead of a script element.
