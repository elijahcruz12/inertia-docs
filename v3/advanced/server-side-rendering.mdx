...
<Steps>
    <Step title="Create an SSR entry point">
        Create a `resources/js/ssr.js` file within your Laravel project.
...
<SvelteSpecific>To enable client-side hydration in a Svelte app, update your `app.js` file to use `hydrate` instead of `mount` when server rendering.</SvelteSpecific>
