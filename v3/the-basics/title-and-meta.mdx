...
[Server-side rendering](/v3/advanced/server-side-rendering) responses include their own `<title>` and `<meta>` tags from the `<Head>` component. A `<title>` tag in your root Blade template would result in duplicate tags in the rendered HTML.

The `<x-inertia::head>` Blade component solves this by accepting fallback content via its slot. This content only renders when SSR is not active.
