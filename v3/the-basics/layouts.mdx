...
The callback receives the page's props and should return any valid layout format, including a single component, a tuple with static props, an array for nested layouts, or a named layout object. TypeScript users may use the [`LayoutCallback`](/v3/advanced/typescript#layout-callbacks) type for type safety.
